...
void update(int node, int st, int en, int idx, int val) {
    ...
    int mid = (st + en) / 2; // traversing to req node
    if (idx <= mid) {
    ...
    tree[node] = tree[2 * node] + tree[2 * node + 1]; // updating while backtracking/recursion property I learnt during trees
}

int rangesum(int node, int st, int en, int l, int r)
{
    if(st > r || en < l) return 0; // no overlap se l  r (or es)   fix ls and put se at various places

    if(l <= st && en <= r) return tree[node]; // l  st  en  r   complete overlap
