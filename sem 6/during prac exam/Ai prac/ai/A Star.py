...
# lambda x: x[4]: This is a lambda function that takes an element x from the list open and returns the value at index 4 of that element. 
# In Python, indexing starts from 0, so x[4] refers to the fifth element (index 4) of the element x.

# sort: The sort method is called on the list open, and it takes a key parameter. The key parameter is set to the lambda function 
# lambda x: x[4], which means that the list will be sorted based on the values returned by the lambda function for each element.